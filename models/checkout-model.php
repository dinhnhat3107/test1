        ...
        if(!empty($userId) && is_numeric($userId)){
            if(!empty(trim($fullname)) && !empty(trim($address)) && !empty(trim($numberphone))){
                // Thêm thông tin người dùng
                    ...
                    $createOrder = $this->db->prepare("INSERT INTO orders(`userId`,`createdate`,`total`,`process`,`status`,`address`, `province`,`district`,`wards`) VALUES (?,?,?,?,?,?,?,?,?)");
                    $createOrder->bind_param("isisssiii", $userId, $createdate, $total, $process, $status, $address, $province, $district, $wards);
                    if($createOrder->execute()){
                        $orderId = $createOrder->insert_id; // Lấy id của đơn hàng vừa thêm
                        // Lấy tất cả sản phẩm trong giỏ hàng
                        $allCart = $this->db->prepare(
                            "SELECT carts.productId AS productId, carts.quantity AS quantity, products.price AS price, products.discount AS discount
                            FROM carts
                            INNER JOIN products 
